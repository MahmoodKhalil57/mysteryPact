...
export const handleRequest = async (url: URL, request: Request, cookies: Cookies) => {
	...
	if (zodValidation) {
		let safeparseRes = await zodValidation.safeParseAsync(params.data);
		...
		if (safeparseRes?.success !== true) {
			...
			Object.entries(errors).forEach((error, key) => {
				let [firstErrorKey, errorMessages] = error;
...
export const getError = <S extends responseStatus, B extends any>(status: S, body: B) => ({
	status,
	body
});
